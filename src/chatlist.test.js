...
describe("Sort Button and Next Button", () => {
  ...
  test("should display button with label Next", () => {
    render(<Header totalMessages={messages} sortOrder='asc' currentPage={1} totalPages={2} />);
...
describe("Sort Messages By Date ascending or descending", () => {
  test("should toggle sorted messages by date using Sort button", async () => {
    ...
    render(<App />);
    fireEvent(screen.getByText("Sort by Date(asc)"), new MouseEvent('click', {
      bubbles: true,
      cancelable: true,
    }))
    ...
    fireEvent(screen.getByText("Sort by Date(desc)"), new MouseEvent('click', {
      bubbles: true,
      cancelable: true,
    }))
    expect(screen.getByText("Sort by Date(asc)")).toBeTruthy();
...
describe("Clicking Next or Prev button", () => {
  test("should show new message and remove current message in the page", async () => {
    ...
    expect(screen.queryAllByText(/Monday Dec 17, 2012 at 09:08:37/)).toHaveLength(0);
    fireEvent(screen.getByText("Next >"), new MouseEvent('click', {
      bubbles: true,
      cancelable: true
    }))
    expect(await screen.findAllByText("Sender UUID: 1") && await screen.findByText("18")).toBeTruthy();
    expect(await screen.findByText(/Monday Dec 17, 2012 at 09:08:37/)).toBeTruthy();

    fireEvent(screen.getByText("< Prev"), new MouseEvent('click', {
      bubbles: true,
      cancelable: true
    }))
    expect(await screen.findAllByText("Sender UUID: 2") && await screen.findByText("15")).toBeTruthy();
    expect(await screen.findByText(/Sunday Feb 19, 2012 at 03:42:11/)).toBeTruthy();
  });
});
...
describe("Clicking on delete icon", () => {
  test("should remove the message from the screen", async () => {
    ...
    expect(await screen.findByText("Monday Nov 5, 2012 at 05:37:00")).toBeTruthy();
    
    fireEvent(screen.getByTestId(`delete-icon-43532134353-10`), new MouseEvent('click', {
      bubbles: true,
      cancelable: true
    }))
    expect(screen.queryAllByText("10")).toHaveLength(0);
    expect(screen.queryAllByText(/Monday Nov 5, 2012 at 05:37:00/)).toHaveLength(0);
